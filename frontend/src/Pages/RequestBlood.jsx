    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (patientResponse.status === 201) {
                ...
                if (bloodRequestResponse.status === 201) {
                    const bloodRequestId = bloodRequestResponse.data.bloodRequest?._id; // Extract blood request ID

                    if (!bloodRequestId) {
                        setError("Error: Blood Request ID is missing.");
                        return;
                    }

                    setSuccessMessage("Blood request created successfully!");
                    setTimeout(() => navigate(`/patient-request/${bloodRequestId}`), 2000); // Redirect after 2 sec
                }
            }
        } catch (error) {
            console.error("Error:", error.response?.data || error.message);
